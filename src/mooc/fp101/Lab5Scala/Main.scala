...
object Main extends App {
  ...
  // Added these for testing - adapted from the Haskell version
  def myex0() = {
    def ho = atom(putStr("ho"))
    def hi = atom(putStr("hi"))
    (ho andThen ho andThen ho) andThen (hi andThen hi andThen hi) andThen atom (putStr("\n"))
  }
  
  def myex1() = {
    def ho = atom(putStr("ho"))
    def hi = atom(putStr("hi"))
    (ho andThen ho andThen ho) fork() andThen (hi andThen hi andThen hi) andThen atom (putStr("\n")) 
  }

  myex0 run // hohohohihihi
  
  myex1 run	// hohihohihohi
  
  // The other 2 examples not ported since Scala doesn't have those functions out of the box
