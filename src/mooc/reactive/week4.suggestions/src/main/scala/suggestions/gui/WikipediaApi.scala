...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
    ...
    def sanitized: Observable[String] = {
      obs.map(x => x.split("\\s+").mkString("_"))
    }
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs.map(t => Success(t)).onErrorReturn(t => Failure(t))
    
    /** Alternate implementation using 'materialize' **/
    def recovered2: Observable[Try[T]] = {
      Observable(observer => {
     ...
     */
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = obs.map(x => requestMethod(x).recovered).concat
    // initial implementation that doesn't work
