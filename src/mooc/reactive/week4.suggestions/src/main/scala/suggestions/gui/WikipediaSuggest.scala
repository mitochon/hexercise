  ...
  def top = new MainFrame {
    ...
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      x => x match {
        ...
        // Update the contents of a component called suggestionList every time the observable produces a value.
        case Success(v) => { suggestionList.listData = v; status.text = " " }
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.map(x => suggestionList.selection.items).filter(!_.isEmpty).map(_.head)
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        ...
        case Success(v) => { editorpane.text = v; status.text = " " }
