...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
     ...
     */
    def always[T](value: T): Future[T] = async {
      value
    }
    ...
    def delay(t: Duration): Future[Unit] = Future {
      blocking {
        ...
        // #1 Thread.sleep(t.toMillis)
        // #2
        Try(Await.result(never, t))
      }
    }
  ...
  // copied from the lecture 3.5
  // put the implementation here due to bug in scala 'nested class not allowed in value class'
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      ...
      f onComplete {
        case Failure(e) => p failure e
        case Success(x) => p complete Try(cont(Try(x))) 
      }
      p.future
    }
  }
  ...
  object CancellationTokenSource {
    ...
    def apply(): CancellationTokenSource = new CancellationTokenSource {
      ...
      def unsubscribe = {
        p completeWith Future.always(1)
      }
    }
  }
}
