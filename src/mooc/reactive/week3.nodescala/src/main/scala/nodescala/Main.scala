...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val myServerSubscription = myServer.start("/test") {
      req => (for ((k, v) <- req) yield (k + " -> " + v)).iterator
    }

    // TO IMPLEMENT
    // 2. create a future that expects some user input `x`
    //    and continues with a `"You entered... " + x` message
    val userInterrupted: Future[String] = Future.userInput("Type something ..") continueWith { x => "You entered... %s".format(x.value.get.get) }
